...
try{
	$job = Move-PnPFile -SourceUrl "/sites/migration/Clients/Existing/$($client)" -TargetUrl "/sites/company/Clients%20 Existing" -NoWait -Force
}
catch{
...
while($jobStatus.JobState -eq 4){
	Write-Host "Getting job status for $($client)..." -ForegroundColor Yellow
